<template>
    <header>
      ...
      <div class="main-nav">
        <nuxt-link to="/new-project">Новый проект</nuxt-link>
        <nuxt-link to="/projects">Мои проекты</nuxt-link>
      </div>
          <div class="user-nav" v-on:click.prevent="showDropDown=!showDropDown">
            <template v-if="$auth.$state.loggedIn">
              <div class="username" >{{$auth.user.username}}</div>
              <!-- <div>{{ remains }}</div> -->
              <transition name="slide-fade">
                    <div class="dropdown-profile-menu shadow-sm" v-if="showDropDown">
                      <b-list-group>
                          <b-list-group-item to="/login" @click="userLogout">Выйти</b-list-group-item>
                      </b-list-group>
                    </div>
              </transition>
              <b-img :src="picture" class="userpic" rounded="circle" width="30px" height="30px" />
            </template>
            <template v-else>
...
<script>
import logosign from '~/assets/img/logo.png'
