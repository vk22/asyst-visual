  ...
  <div class="visualizer-wrap">
    <div id="print" class="main-zone" :class="{'show': allIsShow, 'start': activeStage == 'start'}" :style="{'height': containerHeight+'px'}">
      ...
      <div class="material-container" :class="{'down': activeStage != 'add-material'}" v-for="(item, index) in surfaces" :key="index" :style="{'clip-path':'url(#'+item.clip+')'}">
          <div v-if="item.material.img != undefined" class="material-item" @click="editHoveredSurface(item)" @mouseover="showHoveredSurface(item)" @mouseleave="showHoveredSurface(null)" :id="'material-'+item.id" :style="{backgroundImage: 'url('+ item.material.img +')', transform: 'matrix3d(' + item.matrix + ') scale(1)', backgroundSize: projectScaleProcent}"></div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getPerspectivaHandlerTargetPoints () {
      ...
      let activePoints = this.areas[this.activeAreaIndex].surfaces[this.activeSurface].points[0]
      ...
      console.log('activePoints ', activePoints)
          // if (this.areas[this.activeAreaIndex].surfaces[this.activeSurface].perspectiva != '') {
    ...
    onResize() {
      ...
      d3.selectAll("#persectivaContainer svg#persectivaHandler circle")  
        ...
        .attr("cx", d => 
        { 
          // console.log('persectivaHandler d.x ', d[0])
          // console.log('_this.containerWidth ', _this.containerWidth)
          var newCX = _this.x(d[0]/_this.containerWidth)
          // console.log('persectivaHandler newCX ', newCX)
          return newCX
        })
    ...
    perspectivaTransformed() {
      ...
      for (var a = [], b = [], i = 0, n = this.sourcePoints.length; i < n; ++i) {
        var s = this.sourcePoints[i], t = this.targetPoints[i];
      ...
      console.log('transformed matrix ', matrix)
      //_this.matrix = matrix
      this.areas[this.activeAreaIndex].surfaces[this.activeSurface].perspectiva = targetPoints
   ...
   hoveredSurfaceFromMenu: function (newVal, oldVal) {
    ...
    activeStage: function (newVal, oldVal) {
      ...
      if (newVal == 'edit-surface') {
        ...
        // this.canDrawPath = true;
        // this.initD3Project()
        d3.select('image').attr("xlink:href", this.imageOriginal.url) 
        this.canEditPath = false
        this.canCutPath = false;
        this.showAllCircles()
          d3.selectAll('path')
